//a tool for making horizontal and vertical symmetric drawing

function mirrorDrawTool() {
	//set an icon and a name for the object
	this.name = "mirrorDraw";
	this.icon = "assets/tools/mirrorDraw.svg";
	...
	this.draw = function() {
		...
		if (mouseIsPressed) {
			...
			//draw a line between them and the current positions
				...
				currentLayerPixels.line(previousOppositeMouseX, previousOppositeMouseY, oX, oY);
	...
	this.loadMirrors = function() {
		//Create horizontal and vertical mirrors
		let horizontalMirror = createDiv("<img src='assets/properties/mirrorV.svg' alt='horizontal mirror icon'/>");
		let verticalMirror = createDiv("<img src='assets/properties/mirrorH.svg' alt='vertical mirror icon'/>");

		//Create container for the two mirrors
		let mirrorContainer = createDiv("");
		mirrorContainer.id("mirror-container");
		mirrorContainer.parent(optionsSection);

		let options = [horizontalMirror, verticalMirror];
		...
		for(let option = 0; option < options.length; ++option) {
			options[option].parent(mirrorContainer);
			options[option].addClass("prop-square-btn");
		}

		//Setting horizontal mirror to be active by deafault
		horizontalMirror.addClass('active');
		

		//Event listeners for the options
		horizontalMirror.mouseClicked(function() {
			self.axis = "x";
			self.lineOfSymmetry = width / 2;
			setActiveState(horizontalMirror);
		})

		verticalMirror.mouseClicked(function() {
			self.axis = "y";
			self.lineOfSymmetry = height / 2;
			setActiveState(verticalMirror);
		})
	}
	...
	this.populateOptions = function() {
		...
		//loading all the tool options into the section
		this.loadMirrors();		

	};

	//when another tool is selected update the pixels to just show the drawing and
