...
function Toolbox() {
	...
	var toolbarItemClick = function() {
		...
		for (var i = 0; i < items.length; i++) {
			// items[i].style('background', '#1c1b26');
	...
	var addToolIcon = function(icon, name) {
		var toolBarItem = createDiv("<img src='" + icon + "'/>");
		toolBarItem.class('toolBarItem')
		toolBarItem.id(name + "toolBarItem")
		toolBarItem.parent("toolBar");
		toolBarItem.mouseClicked(toolbarItemClick);
	};
	...
	this.selectTool = function(toolName) {
		...
		for (var i = 0; i < this.tools.length; i++) {
			if (this.tools[i].name == toolName) {
				...
				select("#" + toolName + "toolBarItem").addClass('active');
